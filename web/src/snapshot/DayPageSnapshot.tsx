      ...
      <div class="snapshot-timeline-content" style={{ height: `${PAGE_TIMELINE_HEIGHT}px`, transform: `translateY(${-crop.top}px)` }}>
        ...
        {state.notes.map((note, index) => {
          ...
          return <div class="snapshot-note-position" style={{ left: `${note.author === "master" ? 58 : 214}px`, top: `${note.y ?? 34 + index * 116}px` }} key={note.id}>
            <TornNote note={note} index={index} linkedTitle={linkedTitle} active={false} dragging={false} offsetY={0} onText={() => undefined} onDelete={() => undefined} onDoubleTap={() => undefined} onPointerDown={() => undefined} onPointerMove={() => undefined} onPointerUp={() => undefined} onPointerCancel={() => undefined} onFocusRequest={() => undefined} onFocusChange={() => undefined} />
          </div>;
        })}
