...
function eventTimeLabel(event: DayEvent): string {
  if (event.isAllDay) return event.isSpan && event.spanIndex && event.spanLength
...
function stripDays(dateKey: string): string[] {
  return Array.from({ length: 7 }, (_, index) => adjacentDayKey(dateKey, index - 3))
    .filter((value): value is string => value !== null);
}

function EventDetail({ event, dateKey, onClose }: { event: DayEvent; dateKey: string; onClose(): void }) {
  const parts = parseDayKey(dateKey)!;
  return (
    <div class="event-detail-backdrop" role="presentation" onClick={onClose}>
      <section
        class={`event-detail-sheet ${authorClass(event.author)}`}
        role="dialog"
        aria-modal="true"
      ...
      >
        ...
        <div class="detail-kicker">
          <span>{AUTHOR_LABEL[event.author]}</span>
          {event.eventType && <span>{event.eventType.toUpperCase()}</span>}
        </div>
        <h2 id="event-detail-title">{event.title}</h2>
        <dl>
          <div>
            <dt>DATE</dt>
            <dd>{parts.year}-{String(parts.month).padStart(2, "0")}-{String(parts.day).padStart(2, "0")} · {weekdayLabelForDay(dateKey)}</dd>
          </div>
          <div>
            <dt>TIME</dt>
            <dd>{eventTimeLabel(event)}</dd>
          </div>
          {event.isSpan && event.spanIndex && event.spanLength && (
            <div>
              <dt>SPAN</dt>
...
export function DayPage({ config, dateKey, onBack, onDayChange }: DayPageProps) {
  const month = monthFromDayKey(dateKey)!;
  const parts = parseDayKey(dateKey)!;
  const { status, dtos, unseenDays, message, retry } = useCalendarMonth(config, month);
  ...
  const visibleTimed = payload.timed.filter(
    (event) => event.endMinute > FIRST_HOUR * 60 && event.startMinute < 1440
  );

  useEffect(() => setSelectedEvent(null), [dateKey]);

  return (
    <CanvasViewport>
      <main class="calendar-canvas day-canvas" aria-busy={status === "loading"}>
        <header class="day-back-header">
          <button type="button" onClick={onBack} aria-label={`Back to ${monthLabel(month)}`}>
            <span>‹</span>{monthLabel(month)}
          </button>
        </header>
        ...
        <nav class="day-strip" aria-label="Nearby dates">
          {strip.map((key) => {
            ...
            return (
              <button
                type="button"
                class={[selected ? "is-selected" : "", inMonth ? "" : "is-dim"].join(" ")}
                onClick={() => onDayChange(key)}
                aria-current={selected ? "date" : undefined}
                key={key}
              >
                <span>{weekdayLetterForDay(key)}</span>
                <i>
                  {selected && <img src="/assets/pink-date-circle.png" alt="" aria-hidden="true" />}
                  <b>{stripParts.day}</b>
                </i>
              </button>
            );
        ...
        <section class="day-title-row">
          <div>
            <h1>{parts.month}月{parts.day}日</h1>
            <p>{weekdayLabelForDay(dateKey)}{today ? " · today" : ""}</p>
          </div>
          {unseen && <img class="day-new-badge" src="/assets/red-exclaim-double.png" alt="NEW" />}
        </section>

        {status === "error" && (
          <button class="day-status is-error" type="button" onClick={retry}>{message}</button>
        )}
        {status === "loading" && <p class="day-status">syncing…</p>}

        {payload.allDay.length > 0 && (
          <section class="all-day-rows" aria-label="All-day events">
            {payload.allDay.map((event) => (
              <button
                type="button"
                class={`all-day-event ${authorClass(event.author)}`}
                key={event.id}
                onClick={() => setSelectedEvent(event)}
              >
                <strong>{event.title}</strong>
                <span>{eventTimeLabel(event)} · {AUTHOR_LABEL[event.author]}</span>
                {event.eventType && SPECIAL_DAY_TYPES.has(event.eventType) && (
                  <img src="/assets/stamp-heart-mini.png" alt="" aria-hidden="true" />
                )}
              </button>
            ))}
          </section>
        ...
        <section class="day-timeline" aria-label="Timeline from 06:00 to 23:00">
          <div class="timeline-content" style={{ height: `${TIMELINE_HEIGHT}px` }}>
            ...
            {Array.from({ length: LAST_HOUR - FIRST_HOUR + 1 }, (_, index) => FIRST_HOUR + index).map((hour) => (
              <div class="hour-row" style={{ top: `${10 + (hour - FIRST_HOUR) * ROW_HEIGHT}px` }} key={hour}>
            ...
            {visibleTimed.map((event) => (
              <button
                type="button"
                class={`timed-event ${authorClass(event.author)}`}
                style={eventStyle(event)}
                key={`${event.id}-${event.startMinute}`}
                onClick={() => setSelectedEvent(event)}
              >
                <strong>{event.title}</strong>
                <span>{eventTimeLabel(event)} · {AUTHOR_LABEL[event.author]}</span>
            ...
            {status === "ready" && payload.allDay.length === 0 && payload.timed.length === 0 && (
              <div class="day-empty">
                <span>這一天還空著</span>
        ...
        {selectedEvent && (
          <EventDetail event={selectedEvent} dateKey={dateKey} onClose={() => setSelectedEvent(null)} />
        )}
      </main>
    </CanvasViewport>
  );
